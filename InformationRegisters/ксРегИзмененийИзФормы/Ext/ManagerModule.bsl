...
Процедура ЗаписатьЛог(сс,лог=Неопределено) Экспорт
	...
	СткТип = ПолучитьСткТип(сс);
	
	текХЭШ = ХЭШ(сс,СткТип);
	...
	Зап.идДок = ид;  
	Зап.Прочитать();
	Если зап.хэш = текХЭШ Тогда Возврат; КонецесЛИ;
	...
	РегистрыСведений.ксОчередьОбменов.ОбновитьИдИзменния(сткТип.Тип);
...
Функция ПолучитьСткТип(сс)  
	
	тип     = СтрЗаменить(XMLТипЗнч(сс).ИмяТипа,".",Символы.ПС); 
	ВидМета = СтрПОлучитьСтроку(тип,1);
	тип     = СтрПОлучитьСтроку(тип,2);
	
	Возврат Новый Структура("Тип,ВидМета",Тип,ВидМета);
	
КонецФункции

Функция ХЭШ(сс,СткТип) 
	
	
	СткПар = ПолучитьПарОбк(СткТип.ВидМета,СткТип.тип); 
