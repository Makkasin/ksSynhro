...
Процедура ЗагрузитьОбъекты(ВидОбъекта,ВсеОбъекты=Ложь) Экспорт
	  ...
	  Если ВсеОбъекты = Ложь Тогда
		  Пар.Вставить("_ТблХЭШ",РегистрыСведений.ксРегИзмененийИзФормы.ПолучитьТБлХЭШ(видМета,ВидОбъекта));	  
	  КонецЕсли;
	  
	  Стк.Вставить("пар",пар);  
	  
	  Рез = СделатьЗапрос("SYNHRO",Стк); 
	  итМас = Рез.ИтМас;
	  ЗагрузитьОбк(итМас); 
	  
	  Сообщить("Загружено: "+ВидОбъекта+" - "+итМас.Количество()); 
	  
	  УдалитьЛишниеОбъекты(""+видМета+"."+ВидОбъекта);
	  РегистрыСведений.ксОчередьОбменов.ЗаписатьИдИзменения(Рез.СткИдОчереди);
	
 КонецПроцедуры  
  
Процедура ВыполнитьСинхронизациюИзОчереди() Экспорт	
	рез = РегистрыСведений.ксОчередьОбменов.ПроверитьСтатусыИд();
	Для каждого эл из Рез Цикл
		 ЗагрузитьОбъекты(эл);
	 КонецЦикла;                            
КонецПроцедуры
	
Процедура УдалитьЛишниеОбъекты(ВидМетаОбъекта) Экспорт
	  
	  Стк = новый Структура();
	  Стк.вставить("ВидМета",ВидМетаОбъекта);
	  
	  идМас = СделатьЗапрос("GETALLID",Стк);
	  Если идМАс.Количество()=0 ТОгда Возврат; КонецеслИ;
	  
	  Запрос = новый Запрос;
	  Запрос.УстановитьПараметр("идМас",идМас);
	  Запрос.Текст = "SELECT ссылка сс FROM "+ВидМетаОбъекта+" Док WHERE УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Док.ссылка) NOT IN (&идМас) и Док.ПометкаУдаления = ложь  ";
	  Тбл = Запрос.Выполнить().Выгрузить();
	  Если Тбл.Количество()=0 ТОгда Возврат; КонецеслИ;
	  
	  Для каждого выб из ТБл Цикл
		  Обк = Выб.сс.ПолучитьОбъект();
		  Обк.установитьПометкуУдаления(Истина);
	  КонецЦикла;
	  
	  
	  Сообщить("Помечено на удаление: "+ВидМетаОбъекта+" - "+Тбл.Количество());
	
КонецПроцедуры


Функция СделатьЗапрос(Метод,Стк) Экспорт 
